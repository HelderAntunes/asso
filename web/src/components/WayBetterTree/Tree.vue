...
export default {
  ...
  methods: {
    ...
    updateGraph(source) {
      ...
      updateLinks.attr('d', (d) => {
        drawLink(originBuilder(d), originBuilder(d), this.layout);
      });
      ...
      const exitingLinksPromise = toPromise(links.exit().transition().duration(this.duration).attr('d', d => drawLink(forExit(d), forExit(d), this.layout))
        .remove());
      ...
      text.attr('x', d => (d.textInfo ? d.textInfo.x : 0))
    ...
    onData(data) {
      ...
      const root = d3.hierarchy(data).sort((a, b) => compareString(a.data.text, b.data.text));
    ...
    clean() {
      ['.linktree', '.nodetree', 'text', 'circle'].forEach((selector) => {
        this.internaldata.g.selectAll(selector).transition().duration(this.duration).attr('opacity', 0)
          .remove();
      });
    },
